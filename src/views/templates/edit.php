...
<form class="edit_form" data-bind="visible: !loadingItem(), submit: saveItem">
	...
	<!-- ko foreach: editFields -->
		<!-- ko if: $data && ( $root[$root.primaryKey]() || editable ) && visible -->
			<!-- ko if: type === 'tab' -->
				<div style="float:left;" data-bind="attr: {class: type + ' new_item', id: tabn}, text: title" onclick="$('.tabs').removeClass('active');$('.' +$(this).attr('id')).addClass('active');"></div>
				<!-- /ko -->
		<!-- /ko -->
	<!-- /ko -->
	...
	<!-- ko foreach: editFields -->
		<!-- ko if: $data && ( $root[$root.primaryKey]() || editable ) && visible -->
			<!-- ko if: type !== 'tab' -->
				<div data-bind="attr: {class: type + ' '+ tab}">
